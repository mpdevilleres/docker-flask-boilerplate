...
from project.user_mgt.forms import LoginForm
    ...
    def test_correct_login(self):
        ...
        with self.client:
            response = self.client.post(
                '/login',
                data=dict(username="test_user", password="test_pass"),
                follow_redirects=True
            )
            self.assertIn(b'Change Password', response.data)
            self.assertIn(b'Log Out', response.data)
            self.assertTrue(current_user.test_user == "test_user")
...
#            self.assertIn(b'You were logged out. Bye!', response.data)
    ...
    def test_validate_success_login_form(self):
        ...
        form = LoginForm(username='test_user', password='test_pass')
    ...
    def test_check_password(self):
        ...
        self.assertTrue(bcrypt.check_password_hash(user.password, 'test_pass'))
    ...
    def test_validate_invalid_password(self):
        ...
        with self.client:
            response = self.client.post('/login', data=dict(
                username='test_user', password='foo_bar'
            ), follow_redirects=True)
        self.assertIn(b'Password did not match', response.data)
